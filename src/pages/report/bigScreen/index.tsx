 ...
 * @LastEditTime: 2024-07-25 10:54:51
 * @FilePath: \配置大屏\src\pages\report\bigScreen\index.tsx
 * @Description: 屏幕表格列表页
...
import { FileTools, guid } from '../../../utils/tools'
import { getList, addPost } from '../../../service/screen'
...
const BigScreen: FC<IBigScreenProps> = ({
  ...
  const handleOk2 = () => {
    form2.validateFields().then((res) => {
      FileTools.readJson(file).then((result: any) => {
        ...
        delete result.pages
        if (result.type === 2) {
          message.warning('主页管理导出的模板不能应用于此')
          return
        }
        addPost({
          pages,
                  ...
                  record.agencyId === -1 && userinfo.roleType !== 2 ? null : (
                    <span
                      onClick={() => {
                        ...
                        history.push(
                          `/frame/configuration?id=${record.id}&type=screen`
                        )
                      }}
                      className='link'>
                      编辑
  ...
  const addProject = () => {
    ...
    history.push('/frame/configuration?type=screen')
  }
  return (
    <>
      <Modal
