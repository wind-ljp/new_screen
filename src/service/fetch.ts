...
axios.interceptors.request.use(
  (config: any) => {
    ...
    if (typeof config.headers === 'object') {
      ...
      if (typeof config.noToken === 'boolean' && config.noToken) {
      } else {
        config.headers.authorization = `${session.getItem('token')}`
        config.headers.agency_code = session.getItem('defaultUnitId')
      }
    }
    // 设置cancelToken对象
...
axios.interceptors.response.use(
  ...
  (error: any) => {
    ...
    switch (status) {
      ...
      case 403:
        clearAllFn()
        antdMessage.error('403')
        break
      case 404:
        antdMessage.error('404')
        break
      // case 500:
      //   antdMessage.error('500')
      //   break
      case 503:
        antdMessage.error('503')
        break
      default:
        break
    }
 ...
 * delete请求
 */
export function del(config: IDefaultConfig) {
  return request({
    ...config,
    method: 'delete'
  })
